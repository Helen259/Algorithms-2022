...
abstract class AbstractBinarySearchTreeTest {
    ...
    protected fun doRemoveTest() {
        ...
        for (iteration in 1..1) {
            val controlSet = mutableSetOf<Int>()
            val toRemove = 3
            controlSet.addAll(listOf(8, 3, 10, 1, 6, 4, 7, 10, 14, 13))
    ...
    protected fun doIteratorTest() {
        ...
        controlSet.addAll(arrayOf(12, 20, 60, 90, 106, 204, 250, 350, 600))
        println("Control set: $controlSet")
        val binarySet = create()
        assertFalse(
        ...
        for (iteration in 1..100) {
            ...
            assertFalse(
                binarySet1.iterator().hasNext(),
                "Iterator of an empty tree should not have any next elements."
            )
            for (element in controlSet1) {
                binarySet1 += element
            }
            val iterator11 = binarySet.iterator()
            val iterator22 = binarySet.iterator()
            println("Checking if calling hasNext() changes the state of the iterator...")
            while (iterator11.hasNext()) {
                assertEquals(
                    iterator22.next(), iterator11.next(),
                    "Calling BinarySearchTreeIterator.hasNext() changes the state of the iterator."
                )
            }
            val controlIter1 = controlSet1.iterator()
            val binaryIter1 = binarySet1.iterator()
            println("Checking if the iterator traverses the tree correctly...")
            while (controlIter.hasNext()) {
                assertEquals(
                    controlIter1.next(), binaryIter1.next(),
                    "BinarySearchTreeIterator doesn't traverse the tree correctly."
                )
            }
            assertFailsWith<NoSuchElementException>("Something was supposedly returned after the elements ended") {
